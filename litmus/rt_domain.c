...
static enum hrtimer_restart on_release_timer(struct hrtimer *timer)
{
	...
	litmus_spin_lock_irqsave(&rh->dom->release_lock, flags);
	VTRACE("CB has the release_lock 0x%p\n", &rh->dom->release_lock);
	/* remove from release queue */
	list_del(&rh->list);
	litmus_spin_unlock_irqrestore(&rh->dom->release_lock, flags);
		   ...
		   )
{
	...
	litmus_spin_lock_init(&rt->ready_lock);
	litmus_spin_lock_init(&rt->release_lock);
	litmus_spin_lock_init(&rt->tobe_lock);
